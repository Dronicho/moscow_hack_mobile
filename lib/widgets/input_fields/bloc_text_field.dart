...
class BlocTextField<B extends BlocBase<S>, S> extends HookWidget {
  ...
  final InitValueGetter<S, String?>? titleGetter;
  final String title;
  ...
  Widget build(BuildContext context) {
    final controller = useTextEditingController(text: valueGetter(context.read<B>().state));

    return BlocBuilder<B, S>(
      buildWhen: buildWhen ?? (previous, current) => valueGetter(previous) != valueGetter(current),
      builder: (context, state) {
        var effectiveTitle = title;
        if (titleGetter != null) {
          effectiveTitle = titleGetter!.call(context.read<B>().state)!;
        }
        Widget child;
        child = TextFormField(
